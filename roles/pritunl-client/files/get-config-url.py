...
import sys, base64, hashlib, hmac, random, requests, time, uuid
...
class Pritunl:
    ...
    class KeyClass:
        ...
        def get(self, org_id=None, usr_id=None, file_type=None):
            try:
                if org_id and usr_id and file_type:
                    self.r = self.root.auth_request(method="GET", path="/key/{0}/{1}.{2}".format(org_id, usr_id, file_type))
    ...
    class UserClass:
        ...
        def get(self, org_id=None, usr_id=None, search=None):
            try:
                if org_id and not usr_id and not search:
                ...
                elif org_id and not usr_id and search:
                    self.r = self.root.auth_request(method="GET", path="/user/{0}".format(org_id), params={'search': search})
                if self.r.status_code == 200:
                    return self.r.json()
                else:
                    raise PritunlErr("{0}:{1}".format(sys._getframe().f_code.co_name, self.root.BASE_URL))
    ...
    def ping(self):
        try:
            ...
            if self.r.status_code == 200:
                ...
                raise PritunlErr("{0}:{1}".format(sys._getframe().f_code.co_name, self.BASE_URL))
        except Exception:
            raise PritunlErr("{0}:{1}".format(sys._getframe().f_code.co_name, self.BASE_URL))
    ...
    def auth_request(self, method, path, headers=None, data=None, params=None):
...
def main(argv):
    ...
    parser.add_argument('-url', help='Pritunl server url', required=True)
    parser.add_argument('-token', help='Pritunl API token', required=True)
    parser.add_argument('-secret', help='Pritunl API secret', required=True)
    parser.add_argument('-organization', help='Organization to use', required=True)
    parser.add_argument('-user', help='User to use in Pritunl', required=False, default=socket.gethostname())
    args = parser.parse_args()

    pri = Pritunl(url=args.url,
            token=args.token,
            secret=args.secret)
         
    # Ping host
    if pri.ping():
        servername = args.user
        
        # Find the right organization
        orgs = pri.organization.get()
        organization = next((org for org in orgs if org["name"] == args.organization), None)
        if (not organization):
            raise Exception("Organization " +args.organization + " could not be found in Pritunl.")

        # Find the right user
        q = pri.user.get(org_id=organization['id'], search=servername)
        user = next((item for item in q['users'] if item["name"] == servername), None)
        if (not user):
            raise Exception("User " +args.user + " could not be found in Pritunl.")

        # Get key from user
        key = pri.key.get(org_id=user['organization'], usr_id=user['id'])
        
        # Get hostname
        parsed_uri = urlparse(args.url)
        
        URI = "pritunl://" + parsed_uri.netloc + key.json()['uri_url']
